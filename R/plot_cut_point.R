...
#'@param plot A logical argument for generating the plot. If FALSE, only the cut point value from the GMM is returned. If TRUE, histogram with the overlaid cut point is returned. Default is set to TRUE.
#'@param color A vector of color options including "amerika" (from package \code{amerika}), "wesanderson" (from package \code{wesanderson}), and "grayscale", which is the default option.
